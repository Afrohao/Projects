---
title: 'STAT636: Homework 1'
subtitle: Simulation and Imputation 
output:
...
Write down a sentence comparing Sigma to this sample covariance matrix.

The covariance matrix estimates Sigma, but with n observations and 6 parameters, the estimator still has a lot of variance.
...
# Transform x so that it's rows, X_i, are drawn from N(mu,Sigma)
x_svd = x %*% SigmaSqRt_svd ### #### Answer 1b.1
x_svd = x_svd + mu  ###
...
Write down a sentence comparing the Sigma to this sample covariance matrix.

The covariance matrix estimates Sigma, but with n observations and 6 parameters, the estimator still has a lot of variance.  Note that additionally, this sample covariance matrix is very similar to the one based on the Cholesky decomposition.

## 1c. 

Comparison via the law of large numbers
Let's demonstrate via simulation that the two methods come up with the same square root.  Note that we will ignore the mean `mu' for this comparison
as it doesn't impact the covariance
```{r}
...
for(nSweep in nGrid){
  ...
  cov_chol = cov( xSweep %*% SigmaSqRt_chol ) #### Answer 1c.1
...
meanModeImputeF = function(X){
  ...
  for(j in 1:ncol(X)){
    ...
    if(is.character(x_j) | is.factor(x_j)){
      Ximpute[,j] = modeImputeF(x_j) #### Answer 2.2
    }
    if(is.numeric(x_j)){ 
      Ximpute[,j] = meanImputeF(x_j) #### Answer 2.3
    }else{
      break
...
iterativeF = function(X, maxIters = 10, threshold = 1, verbose = TRUE){
  ...
  Ximpute         = meanModeImpute$Ximpute #### Answer 2.4
  M               = meanModeImpute$M
...
# Simulate some data
```{r problem2simulation}
set.seed(1)
p               = 2
n               = 100
covarianceMat   = matrix(c(1,.75,.75,1),nrow=2)
X_cQuantitative = matrix(rnorm(n*p),nrow=n) %*%  chol(covarianceMat) #### Answer 2.6
...
```{r problem2table}
M = is.na(X$x3)
tbl = table(Xhat$x3[M],X_c$x3[M])

correctImputation   = sum( diag(tbl) )  #### Answer 2.7  Fill in the value from your imputations
```


I correctly imputed `r correctImputation` observations out of `r sum(M)` observations 